...
def tight_face_mask(img):
    ...
    # remove eyes/mouth/top/neck (basic)
    ...
    # Slightly shrink mask to limit edits strictly to the face
    shrink_px = max(3, int(min(w,h)*0.012)) | 1  # odd size for MinFilter
    try:
...
def main():
    ...
    # base image (A-pass enhancement): unsharp only on face area
    ...
    # composite sharpened content only where face mask is present
    base_enhanced = Image.composite(img_sharp, img, face)
    base_enhanced.save(base_path)

    # contour_map for ControlNet2 (thin black lines on white)
    ...
    # Cheekbone accent lines (upper under-eye and main cheek line)
    ...
    # razor ridge highlight just above cheek line
    ...
    # Eyelids (upper and lower) and slight crow's feet
    ...
    # Nose: bridge and wings (sides already present, add bridge and wing hints)
    # Nose glossy highlight along the bridge (slightly thicker) and tip highlight
    ...
    # Lips contours
    # Stronger lip liner (thicker strokes + cupid bow hint)
    ...
    # Under-chin line and neck shading (keep within face area)
    ...
    inv = ImageOps.invert(lines)  # lines white on black -> invert to make lines black?
    # Actually 'lines' is white lines on black; create final as 255 white background minus lines*alpha
    contour = Image.composite(Image.new("L",(w,h),0), white, lines)  # darker lines on white
    # Ensure crispness
